      ...
      <div style={{ width: 400, maxWidth: '90%' }}>
        ...
        <form onSubmit={handleRegister} style={{ background: 'var(--surface)', padding: 32, borderRadius: 16, border: '1px solid var(--border)' }}>
          ...
          <div style={{ marginBottom: 16 }}>
            <label style={{ display: 'block', fontSize: 13, color: 'var(--text2)', marginBottom: 6 }}>{t('auth.name')}</label>
          ...
          <div style={{ marginBottom: 24 }}>
            <label style={{ display: 'block', fontSize: 13, color: 'var(--text2)', marginBottom: 6 }}>{t('auth.password')} (min 6)</label>
          ...
          <p style={{ textAlign: 'center', marginTop: 16, fontSize: 13, color: 'var(--text2)' }}>
            {t('auth.have_account').split('?')[0]}? <Link href="/auth/login" style={{ color: 'var(--primary)' }}>{t('auth.have_account').split('? ')[1] || 'Sign in'}</Link>
          </p>
        </form>
      </div>
    </div>
