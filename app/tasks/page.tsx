...
export default function TasksPage() {
  ...
  const [form, setForm] = useState({ name: '', keywords: '', platforms: [] as string[], targetCount: 100 });
  ...
  const createTask = async () => {
    ...
    if (res.ok) {
      setShowModal(false);
      setStep(1);
      setForm({ name: '', keywords: '', platforms: [], targetCount: 100 });
      loadTasks();
    }
  };
    ...
    <DashboardLayout>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        <div>
          ...
          <p style={{ color: 'var(--text2)', margin: '4px 0 0' }}>Create and manage lead search tasks</p>
        </div>
      ...
      {showModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
          <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 16, padding: 32, width: 500, maxWidth: '90%' }}>
            <h2 style={{ marginTop: 0 }}>New Search Task</h2>
            {step === 1 && (
              <>
